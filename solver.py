...
def hand_rank(cards):
    vals = sorted([rank_map[c[1]] for c in cards], reverse=True)
    suits_ = [c[0] for c in cards]
    ...
    if straight and flush:
        return (8, top)                       # 同花順
    if freq == [4, 1]:                        # 四條
        four   = max(counts, key=lambda v: counts[v]==4)
        kicker = max(v for v in vals if v != four)
        return (7, four, kicker)
    if freq == [3, 2]:                        # 葫蘆
        three = max(v for v,c in counts.items() if c==3)
    ...
    if freq == [2, 2, 1]:                     # 兩對
        ...
        kicker  = max(v for v in vals if counts[v]==1)
...
def get_specific_hole_combos(hand):
    r1, r2 = hand[0], hand[1]
    if len(hand) == 2:  # 對子
        return [[s1 + r1, s2 + r2] for i, s1 in enumerate(suits) for s2 in suits[i+1:]]
    if hand[2] == 's':  # 同花
        return [[r1 + s, r2 + s] for s in suits]
    if hand[2] == 'o':  # 不同花
        return [[s1 + r1, s2 + r2] for s1 in suits for s2 in suits if s1 != s2]
...
if __name__ == "__main__":
    ...
    for hand in combos:
        ...
        count += 1
        w, t = estimate_equity_montecarlo(hand, iters=10000)
